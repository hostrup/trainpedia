...
- [ ] **M2** [Medium] `src/routes/+page.server.ts:7-21` — Manglende DB fejlhåndtering i server-load. Bevis: `await db.era.findMany(...)` uden try/catch crasher hele siden ved DB-offline. Fix: wrap kald i try/catch med pæn fallback-værdi eller 503 fejlside.
...
- [ ] **M5** [Medium] `src/lib/components/MuseumPlacard.svelte:37` — CSS variable i class-attribut er syntaktisk ugyldig. Bevis: `class="... var(--transition-bezier-heavy) ..."` tolkes som et klassenavn frem for CSS. Fix: flyt timing-funktionen til inline style.
